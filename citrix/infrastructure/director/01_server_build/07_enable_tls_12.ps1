# Thanks go to the below for this script !
# Copyright 2019, Alexander Hass
# https://www.hass.de/content/setup-microsoft-windows-or-iis-ssl-perfect-forward-secrecy-and-tls-12

# Determine where to do the logging
$logPS = "C:\Windows\Temp\enable_tls_12.log"
...
Write-Verbose 'Multi-Protocol Unified Hello has been disabled.' -Verbose
...
Write-Verbose 'PCT 1.0 has been disabled.' -Verbose
...
Write-Verbose 'SSL 2.0 has been disabled.' -Verbose
...
Write-Verbose 'SSL 3.0 has been disabled.' -Verbose
...
Write-Verbose 'TLS 1.0 has been disabled.' -Verbose
...
Write-Verbose 'TLS 1.1 has been disabled.' -Verbose
...
Write-Verbose 'TLS 1.2 has been enabled.' -Verbose
...
Foreach ($insecureCipher in $insecureCiphers) {
  ...
  $key.close()
  Write-Verbose "Weak cipher $insecureCipher has been disabled." -Verbose
}
...
Foreach ($secureCipher in $secureCiphers) {
  ...
  $key.close()
  Write-Verbose "Strong cipher $secureCipher has been enabled." -Verbose
}
...
Foreach ($secureHash in $secureHashes) {
  ...
  $key.close()
  Write-Verbose "Hash $secureHash has been enabled." -Verbose
}
...
Foreach ($secureKeyExchangeAlgorithm in $secureKeyExchangeAlgorithms) {
  ...
  $key.close()
  Write-Verbose "KeyExchangeAlgorithm $secureKeyExchangeAlgorithm has been enabled." -Verbose
}
...
Write-Verbose 'Configure longer DHE key shares for TLS servers.' -Verbose
...
if ([System.Version]$os.Version -lt [System.Version]'10.0') {
  Write-Verbose 'Use cipher suites order for Windows 2008/2008R2/2012/2012R2.' -Verbose
  $cipherSuitesOrder = @(
  ...
  Write-Verbose 'Use cipher suites order for Windows 10/2016 and later.' -Verbose
  $cipherSuitesOrder = @(
...
Write-Verbose 'Enable TLS 1.2 for .NET 3.5 and .NET 4.x' -Verbose
...
if ([System.Version]$file_version_winhttp_dll -lt [System.Version]"6.1.7601.23375" -or [System.Version]$file_version_webio_dll -lt [System.Version]"6.1.7601.23375") {
  Write-Verbose 'WinHTTP: Cannot enable TLS 1.2. Please see https://support.microsoft.com/en-us/help/3140245/update-to-enable-tls-1-1-and-tls-1-2-as-a-default-secure-protocols-in for system requirements.' -Verbose
} else {
  Write-Verbose 'WinHTTP: Minimum system requirements are met.' -Verbose
  Write-Verbose 'WinHTTP: Activate TLS 1.2 only.' -Verbose
...
Write-Verbose 'Windows Internet Explorer: Activate TLS 1.2 only.' -Verbose
